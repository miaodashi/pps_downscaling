...
class MCA_STM(nn.Module):
    def __init__(self, in_c: int) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(in_c, 1, 3, padding=1)
        self.backbone = MSCAN(in_channels=in_c, embed_dims=[64, 128, 256, 512], mlp_ratios=[4, 4, 4, 4], depths=[3, 4, 6, 3], num_stages=4, attention_kernel_sizes=[5, [1, 7], [1, 11], [1, 21]], attention_kernel_paddings=[2, [0, 3], [0, 5], [0, 10]], act_cfg=dict(type='GELU'), norm_cfg=dict(type='SyncBN', requires_grad=True))
        self.regression = MultiModalRegression(3 + 64)
